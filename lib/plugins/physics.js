...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  bot.lastSentYaw = null
  bot.lastSentPitch = null
  ...
  function updatePosition (now) {
    ...
    const dYaw = deltaYaw(bot.entity.yaw, bot.lastSentYaw)
    const dPitch = bot.entity.pitch - (bot.lastSentPitch || 0)
    ...
    bot.lastSentYaw += math.clamp(-maxDeltaYaw, dYaw, maxDeltaYaw)
    bot.lastSentPitch += math.clamp(-maxDeltaPitch, dPitch, maxDeltaPitch)

    const yaw = Math.fround(conv.toNotchianYaw(bot.lastSentYaw))
    const pitch = Math.fround(conv.toNotchianPitch(bot.lastSentPitch))
  ...
  bot.on('move', () => {
    if (!lookingTask.done && Math.abs(deltaYaw(bot.entity.yaw, bot.lastSentYaw)) < 0.001) {
      lookingTask.finish()
    }
  })
  ...
  bot._client.on('position', (packet) => {
    ...
    bot.jumpTicks = 0
    bot.lastSentYaw = bot.entity.yaw
    bot.lastSentPitch = bot.entity.pitch
